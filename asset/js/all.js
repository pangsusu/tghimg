...
function render_uploaded() {
    ...
    uploaded_files.forEach(function (x) {
        var resp = x.resp;
        const _URL = "https://img1.131213.xyz"
        var url = resp.data.thumb == null ? _URL + resp.data.url : _URL + resp.data.thumb.url;
        var img3001 = resp.data.img3001
        var img58 = resp.data.img58
        $('#imagedetail').append(formatHtml({ url: url, code: url, wp: img3001, raw: img58 }));
        $('#htmlcode').append(formatHtml({ url: url, code: '<img src="' + url + '" />', wp: '<img src="' + img3001 + '" />', raw: '<img src="' + img58 + '" />' }));
        $('#bbcode').append(formatHtml({ url: url, code: '[img]' + url + '[/img]', wp: '[img]' + img3001 + '[/img]', raw: '[img]' + img58 + '[/img]' }));
        $('#markdown').append(formatHtml({ url: url, code: '![' + url + '](' + url + ')', wp: '![' + img3001 + '](' + img3001 + ')', raw: '![' + img58 + '](' + img58 + ')' }));
    });
}
...
$smfile.on('fileuploaded', async function (event, data, previewId, index) {
    ...
    // 假设您已经有一个已读取的文件内容的 ArrayBuffer：arrayBuffer

    // 将 ArrayBuffer 转换为 Uint8Array
    let uint8Array = new Uint8Array(reader.result);

    // 将 Uint8Array 转换为字符串
    let binaryString = uint8Array.reduce((data, byte) => {
        return data + String.fromCharCode(byte);
    }, '');

    // 使用 btoa 函数将二进制字符串转换为 Base64 字符串
    let base64String = btoa(binaryString);
    const res58 = await fetch('/58img', {
        method: 'POST',
        body: JSON.stringify({ base64Data: base64String })
    });
    const resultUrl58 = await res58.text();

    if (response.error === undefined) {
        response = { data: { url: response.tgsrc, img3001: response.img3001, img58: resultUrl58 } }
